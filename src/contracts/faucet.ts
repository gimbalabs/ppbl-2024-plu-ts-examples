import { data, pdelay, pfn, pmatch, PScriptContext, pStr, ptraceIfFalse, plet, passert, perror, PMaybe, unit, punsafeConvertType, bs, PTxOutRef, PTxOut, int, pInt } from "@harmoniclabs/plu-ts";
...
((accessTokenSymbol, faucetTokenSymbol, ctx ) => {
    ...
    const ownOutput = plet(
        pmatch(
            tx.outputs.find( output => output.address.eq(ownInput.address) )
        )
        .onJust( ({ val }) => val )
        .onNothing(_ => perror( PTxOut.type ) as any )
    );
    ...
    const faucetTokenOutputContract = plet(
        ownOutput.value.amountOf(faucetTokenSymbol, datum.faucetTokenName).add(datum.withdrawalAmount)
    ...
    const accessTokenOutputPkh = plet(
        ...
        .onNothing(_ => perror( int, "Access token not found in output contract" ))
    ).eq(pInt(1));
    ...
    const faucetTokenOutputPkh = plet(
        ...
        .onNothing(_ => perror( int, "Faucet token not found in output pkh" ) as any )
    ).eq(datum.withdrawalAmount);


    return passert.$(
        (ptraceIfFalse.$(pdelay(pStr("Invalid accessTokenOutputPkh"))).$(accessTokenOutputPkh))
        .and(ptraceIfFalse.$(pdelay(pStr("Invalid faucetTokenOutputPkh"))).$(faucetTokenOutputPkh))
        .and(ptraceIfFalse.$(pdelay(pStr("Invalid faucetTokenOutputContract"))).$(faucetTokenOutputContract))
        .and(ptraceIfFalse.$(pdelay(pStr("Invalid Output Datum"))).$( ownInput.datum.eq(ownOutput.datum)))
    );
